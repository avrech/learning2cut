...
class SCIPTuningDQNLearner:
    ...
    def __init__(self, hparams, use_gpu=True, gpu_id=None, run_io=False, run_setup=False, **kwargs):
        ...
        self.target_net = SCIPTuningQnet(hparams, use_gpu, gpu_id).to(self.device)
    ...
    def sgd_step(self, batch, importance_sampling_correction_weights=None):
        ...
        a_batch = torch.cat([torch.full((len(self.hparams['action_set']),), idx, dtype=torch.long) for idx in range(self.batch_size)]).to(self.device)
