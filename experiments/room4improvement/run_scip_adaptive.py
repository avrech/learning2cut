...
def run_node(args):
    ...
    node_results = {'best_db_aucs': {p: {gs: 0 for gs in insts.keys()} for p, insts in data.items()},
                    'best_configs': {p: {} for p in data.keys()},
                    'configs': {cfg: {} for cfg in node_configs}}
    # wait for logs
    last_save = 0
    ...
    while len(node_results['configs']) < len(node_configs):
        ...
        for cfg, cfg_db_auc_avgs in logs:
            node_results['configs'][cfg].update(cfg_db_auc_avgs)
...
def main(args):
    data, all_configs = get_data_and_configs()
    # load prev rounds best configs
    

    # update main_results
    main_results_file = os.path.join(args.rootdir, 'scip_adaptive_main_results.pkl')
    if os.path.exists(main_results_file):
        ...
        main_results = {'best_db_aucs': {p: {gs: 0 for gs in insts.keys()} for p, insts in data.items()},
                        'best_configs': {p: {} for p in data.keys()},
                        'configs': {}}
    for path in tqdm(Path(args.rootdir).rglob('scip_adaptive_node_results.pkl'), desc='Loading node files'):
